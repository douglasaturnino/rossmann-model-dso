...
class Rossmann( object):
    ...
    def data_preparations(self, df5):
    ...
    def get_prediction(self, model, original_date, teste_data):
        #prediction
        pred = model.predict(teste_data)

        # join pred into the original data
        original_date['prediction'] = np.expm1(pred)

        return original_date.to_json(orient='records', date_format='iso')
